...
DB_COUNT = "SELECT COUNT(*) FROM entries"
...
def show_entry(entry_id):
    if 'logged_in' in session:
        ...
        if request.method == 'GET':
            ...
            try:
                update_entry(
                    entry_id, request.form['title'], request.form['text'])
...
def add_entry():
    if 'logged_in' in session:
        try:
            ...
            # this will catch any errors generated by the database
            abort(500)
        entry_num = count_entries()
        #entry = get_entry(entry_num+32)
        entry = get_entry(entry_num)
        entry['text'] = markdown.markdown(
            entry['text'], extensions=['codehilite'])
        entry['created'] = get_local_datetime(entry['created'])
